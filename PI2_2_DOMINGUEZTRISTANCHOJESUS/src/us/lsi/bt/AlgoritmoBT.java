...
/**
 * <p>
 * Algoritmo que implementa la técnica de Bactracking con sus variantes. Un
 * problema que se quiera resolver con esta técnica debe implementar el
 * interface ProblemaBT &lt; S,A &gt;
 * </p>
 * 
 * <p>
 * La documentación puede encontarse en el:
 * <a href="../../../document/Tema15.pdf" target="_blank">Tema15</a>
 * </p>
 * 
 * @author Miguel Toro
 *
 * @param <S>
 *            El tipo de la solución
 * @param <A>
...
public class AlgoritmoBT<S extends SolucionBT, A> extends AbstractAlgoritmo {
	...
	/**
	 * Número de soluciones que se buscan si no se buscan todas y el problema es de
	 * tipo Otro
	 */
	public static int numeroDeSoluciones = 1;
	/**
	 * Si se quiere aplicar la técnica aleatoria para escoger una de las
	 * alternativas
	 */
	public static boolean isRandomize = false;
	/**
	 * Tamaño umbral a partir del cual se escoge aleatoriamente una de las
	 * alternativas
	 */
	...
	/**
	 * @return El mejor valor de la propiedad objetivo del problema inicial
	 *         encontrado por el algoritmo hasta este momento
	 */
	...
	/**
	 * @param e
	 *            - El esatod inicial del problema a resolver
	 */
	...
	public void ejecuta() {
		if (AbstractAlgoritmo.metricasOK)
			AbstractAlgoritmo.metricas.setTiempoDeEjecucionInicial();
		tipo = estado.getTipo();
		...
		if (AbstractAlgoritmo.metricasOK)
			AbstractAlgoritmo.metricas.setTiempoDeEjecucionFinal();
	}
	...
	private void actualizaSoluciones() {
		...
		if (s != null) {
			...
			if ((this.isTodasLasSoluciones() || this.isAlgunasSoluciones()
	...
	/**
	 * @param p
	 *            - Predicado con las propiedades de las soluciones buscadas
	 * @return Conjunto de soluciones con las propiedades requeridas
	 */
	public Set<S> getSolucionesFiltradas(Predicate<S> p) {
		return soluciones.stream().filter(p).collect(Collectors.<S>toSet());
	}
	...
	public Set<S> getMejoresSoluciones() {
		return soluciones.stream().filter(x -> x.getObjetivo().equals(solucion.getObjetivo()))
