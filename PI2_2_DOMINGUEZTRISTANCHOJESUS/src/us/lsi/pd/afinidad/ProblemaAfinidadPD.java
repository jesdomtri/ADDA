...
public class ProblemaAfinidadPD implements ProblemaPDR<SolucionAfinidad, Integer> {
	...
	public static ProblemaAfinidadPD create(int indexCliente, List<Integer> numClientesPorTrabajador,
			Map<Integer, Set<Integer>> trabajadorOcupadosEnFranja, int afinidadAcum) {
		return new ProblemaAfinidadPD(indexCliente, numClientesPorTrabajador, trabajadorOcupadosEnFranja, afinidadAcum);
	}
	...
	public ProblemaPD<SolucionAfinidad, Integer> getSubProblema(Integer a) {
		...
		Integer afinidadAcum = this.afinidadAcum
	...
	public Sp<Integer> getSolucionParcialPorAlternativa(Integer a, Sp<Integer> sp) {
		return Sp.create(a, sp.propiedad + (this.getTrabajadoresAfinesACliente(indexCliente).contains(a) ? 1 : 0));
	}

	public List<Integer> getAlternativas() {
		List<Integer> ret = IntStream.range(0, ProblemaAfinidad.trabajadores.size()).filter(x -> cumpleRestricciones(x))
	...
	public SolucionAfinidad getSolucionReconstruidaCasoRecursivo(Sp<Integer> sp, SolucionAfinidad s) {
		...
		msol.put(ProblemaAfinidad.clientes.get(indexCliente).nombre, ProblemaAfinidad.trabajadores.get(sp.alternativa));
	...
	public int hashCode() {
		...
		result = prime * result + ((numClientesPorTrabajador == null) ? 0 : numClientesPorTrabajador.hashCode());
		result = prime * result
				+ ((trabajadoresOcupadosEnFranja == null) ? 0 : trabajadoresOcupadosEnFranja.hashCode());
		return result;
	}
	...
	public boolean equals(Object obj) {
		...
		if (numClientesPorTrabajador == null) {
			if (other.numClientesPorTrabajador != null)
				return false;
		} else if (!numClientesPorTrabajador.equals(other.numClientesPorTrabajador))
			return false;
		if (trabajadoresOcupadosEnFranja == null) {
			if (other.trabajadoresOcupadosEnFranja != null)
				return false;
		} else if (!trabajadoresOcupadosEnFranja.equals(other.trabajadoresOcupadosEnFranja))
			return false;
		return true;
	}
	...
	// return "["+indexCliente + ", "+ numClientesPorTrabajador + ", " +
	// afinidadAcum + ", "+ trabajadoresOcupadosEnFranja +"]";
	// }
