...
public class Tree<E> {
	...
	/**
	 * @param <E>
	 *            El tipo de las etiquetas
	 * @param tree
	 *            Una �rbol
	 * @return Un �rbol copia
	 */
	 ...
	 *            Un string
	 * @param fLabel
	 *            Una funci�n que transforma una string en un elemento del tipo E
	 * @return Un �rbol construido a partir de la string
	 */
	 ...
	 * @param ls
	 *            Una lista
	 * @return Un �rbol cuya raiz contiene el primer elemento de la lista, su primer
	 *         hijo el segundo elemento, etc.
	 */
	...
	private Tree() {
		...
		this.elements = new ArrayList<Tree<E>>(INITIAL_CAPACITY);
	...
	private Tree(E label, List<Tree<E>> elements) {
		...
		this.elements = new ArrayList<Tree<E>>();
	...
	private static <E> Tree<E> leeTree(TokenizeString s, Function<String, E> fLabel) {
		...
		List<Tree<E>> ls = new ArrayList<Tree<E>>();
		E label = null;
		Tree<E> t = null;
	...
	private static <E> String arista(Tree<E> t, int index) {
		return "    " + "\"" + t.id + "\"" + " -> " + "\"" + t.getElement(index).id + "\"" + " [label=\"" + index
				+ "\"];";
	}

	/*
	 * public void toDOT(String file, String titulo){ Tree.toDOT(this,file,titulo);
	 * }
	 * 
	 * private static <E> void toDOT(Tree<E> t, String file, String titulo){
	 * AbstractAlgoritmo.setFile(file);
	 * AbstractAlgoritmo.getFile().println("digraph "
	 * +titulo+" {  \n size=\"100,100\"; "); asignarNullAlId(t); Tree.nId = 0;
	 * Tree.toDOTVertex(t); Tree.toDOTEdge(t);
	 * AbstractAlgoritmo.getFile().println("}"); }
	 * 
	 * private static <E> void toDOTVertex(Tree<E> t) { if (t != null && t.id==null)
	 * { t.id = Tree.nId; Tree.nId++;
	 * AbstractAlgoritmo.getFile().println(Tree.label(t)); for (int i = 0; i <
	 * t.getNumChildren(); i++) { Tree.toDOTVertex(t.getElement(i)); } } }
	 * 
	 * private static <E> void toDOTEdge(Tree<E> t){ if (t != null && !t.isLeaf()) {
	 * for (int i = 0; i < t.getNumChildren(); i++) { Tree<E> th = t.getElement(i);
	 * if(th==null) continue; AbstractAlgoritmo.getFile().println(Tree.arista(t,i));
